...
public class HazlcastCacheHelper implements CacheInstance{
	...
	public CacheBean getPcGameLevelBetTypeBean() {
		return BMDataContext.getContext().getBean(PcGameLevelBetTypeCache.class).getCacheInstance(CacheServiceEnum.PC_GAMELEVEL_BETTYPE_CACHE.toString()) ;
	}
	@Override
	public CacheBean getBetValueCacheBean() {
		return BMDataContext.getContext().getBean(PcBetValueCache.class).getCacheInstance(CacheServiceEnum.PC_BETVALUE_CACHE.toString()) ;
	}
	@Override
